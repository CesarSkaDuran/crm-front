...
@Component({
  ...
  templateUrl: './tipos-tercero.component.html',
  styleUrl: './tipos-tercero.component.scss',
})
export class TiposTerceroComponent implements OnInit {
  private fb = inject(FormBuilder);
  private cdr = inject(ChangeDetectorRef);
  private noti = inject(NotificacionesService);
  private service = inject(TiposTerceroService);
  ...
  cargar() {
    this.service.getAll({ search: this.search }).subscribe({
      ...
      error: () => this.noti.error('Error al cargar tipos de tercero'),
    });
  }
